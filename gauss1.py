...
def solution_triangle(A,b):
    """ Renvoie la solution du système Ax =blorsque A est tringulaire supérieure inversible"""
    n =len(b)
...
def solution_systeme(A0,b0):
    ...
    A,b = copy.deepcopy(A0), copy.deepcopy(b0) # on ne détruitpas les données initiales
    n = len(A)# Mise sous forme triangulaire
    for j in range(n-1): # on itère n-1 fois
         ...
         for i in range(j+1,n):
             ...
             transvection(b,i,j,-x) # attention b matrice colonne# phase de remontée
    return solution_triangle(A,b)
#enfait, mon gauss moi meme, ça fonctionne mal, j'ai donc copié l'algo sur internet pour gagner le temps xd
#pourtant ça fonctionne pour généralement
